...
Original file is located at
    https://colab.research.google.com/drive/1Tn1L0ZSEHFxrgmA_U3CcsbwO1MGH7zfx
"""
...
def _load(checkpoint_path):
    if device == 'cuda':
        # If CUDA available, load TorchScript model on GPU
        checkpoint = torch.jit.load(checkpoint_path)
    else:
        # Load on CPU; if TorchScript archive, use torch.jit.load with map_location
        try:
            checkpoint = torch.jit.load(checkpoint_path, map_location='cpu')
        except RuntimeError:
            # fallback to torch.load disabling weights_only for compatibility (trusted source)
